...
const { displayDate, leagueDayKey, nextDayKey } = require('./time');
...
function buildPublicState(state, activeDay) {
  ...
  return {
    ...
    statement,
    question: typeof outcome?.played === 'boolean' ? null : 'did yernar play league?',
    history,
    chart: {
      issued,
...
class LeagueService {
  ...
  async getState() {
    ...
    const mutation = await this.store.update((state) => {
      let changed = finalizeClosedScores(state, activeDay, issuedAt);
  ...
  async setTodayOutcome(played) {
    if (typeof played !== 'boolean') throw new TypeError('played must be a boolean.');
    const now = this.clock();
    const activeDay = leagueDayKey(now, this.timeZone);
    const changedAt = now.toISOString();
    const mutation = await this.store.update((state) => {
      let changed = finalizeClosedScores(state, activeDay, changedAt);
