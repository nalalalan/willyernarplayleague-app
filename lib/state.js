...
function buildPublicState(state, activeDay) {
  ...
  const hasOutcome = typeof outcome?.played === 'boolean';
  ...
  return {
    ...
    statement,
    question: null,
    canRecordOutcome: !hasOutcome,
    canRecordDidNotPlay: !hasOutcome,
    canChangeMind: outcome?.played === false,
    actionLabel: !hasOutcome ? 'no league' : null,
    yesActionLabel: !hasOutcome ? 'yes league' : null,
    changeMindLabel: outcome?.played === false ? 'he changed his mind' : null,
    history,
    chart: {
      activeDay,
