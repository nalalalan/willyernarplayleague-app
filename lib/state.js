...
const { brier, buildRecursiveOutlook, computeForecast, logLoss } = require('./model');
...
function finalizeMissingClosedDays(state, activeDay, finalizedAt) {
  ...
  while (cursor <= lastClosedDay) {
    if (typeof state.outcomes[cursor]?.played !== 'boolean') {
...
function ensureOutlook(state, activeDay, issuedAt, force = false) {
  ...
  if (!force
    ...
    && current.points.length === 7) {
    return false;
  }

  const outlook = buildRecursiveOutlook(state, activeDay, 7);
...
function buildPublicState(state, activeDay) {
  ...
  const issued = Object.values(state.forecasts.official)
    .filter((forecast) => forecast.targetDay <= activeDay)
  ...
  return {
    ...
    statement,
    question: null,
    canRecordDidNotPlay: typeof outcome?.played !== 'boolean',
    actionLabel: typeof outcome?.played !== 'boolean' ? "yernar didn't play league" : null,
    history,
    chart: {
      issued,
...
class LeagueService {
  ...
  async recordTodayNo(expectedLeagueDay) {
    ...
    try {
      ...
      if (previousPlayed === true) {
        const error = new Error('The active League day is already finalized.');
        error.statusCode = 409;
        throw error;
      }

      if (previousPlayed !== false) {
        const sequence = state.nextSequence;
        state.nextSequence += 1;
        state.outcomeRevision += 1;
        ...
        state.audit.push({ sequence, leagueDay: activeDay, played: false, changedAt });
        state.audit = state.audit.slice(-AUDIT_LIMIT);
        changed = true;
      }

      changed = ensureOutlook(state, activeDay, changedAt, previousPlayed !== false) || changed;
      return { changed };
      });
    } catch (error) {
