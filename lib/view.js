...
function renderPrediction(state) {
  ...
  if (state.canRecordOutcome) {
    action = renderOutcomeActions(state.yesActionLabel, state.actionLabel);
  } else if (state.canChangeMind) {
    action = renderChangeMindAction(state.changeMindLabel);
  }
...
function renderOutcomeActions(yesLabel = 'yes league', noLabel = 'no league') {
  return `<div class="answer-buttons">
      <button class="answer-button answer-affirmative" type="button" data-played="true">${escapeHtml(yesLabel)}</button>
      <button class="answer-button answer-exception" type="button" data-played="false">${escapeHtml(noLabel)}</button>
    </div>`;
}

function renderChangeMindAction(label = 'he changed his mind') {
  return `<div class="answer-buttons">
      <button class="answer-button answer-change-mind" type="button" data-played="true">${escapeHtml(label)}</button>
    </div>`;
}
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="/styles.css?v=20260724-outcome-states-v1">
    <script src="/chart.js?v=20260724-ticks-only-v1" defer></script>
    <script src="/app.js?v=20260724-outcome-states-v1" defer></script>
  </head>
  <body>
