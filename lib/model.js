...
function answeredRowsBefore(state, targetDay) {
  return Object.entries(state.outcomes || {})
    ...
    .map(([dateKey, record]) => ({ dateKey, played: record.played, source: record.source || null }));
}
...
function transitionRate(rows, targetDay, center, map = outcomeMapFromRows(rows)) {
...
function streakRate(rows, targetDay, center, map = outcomeMapFromRows(rows)) {
  ...
  for (const row of rows) {
    const isContiguous = previousDate !== null && addDays(previousDate, 1) === row.dateKey;
    const context = isContiguous
      ? { played: previousPlayed, length: previousRunLength }
      : null;
    if (context
      && context.played === targetStreak.played
      && Math.min(context.length, 3) === targetBucket) {
      samples.push({ played: row.played, weight: 1 });
    }
    previousRunLength = isContiguous && previousPlayed === row.played
      ? previousRunLength + 1
      : 1;
    previousDate = row.dateKey;
    previousPlayed = row.played;
  }
...
function gapContextRate(rows, targetDay, center, strength, width) {
  ...
  const contexts = rows.map((row) => {
    const gap = latestYes ? daysBetween(latestYes, row.dateKey) : null;
    if (row.played) latestYes = row.dateKey;
    return { row, gap };
  });
  const targetGap = latestYes ? daysBetween(latestYes, targetDay) : null;
  if (targetGap === null) return center;
  const samples = [];
  for (const { row, gap } of contexts) {
...
function weeklyCadenceRate(rows, targetDay, center, map = outcomeMapFromRows(rows)) {
...
function recentLoadRate(rows, targetDay, center, map = outcomeMapFromRows(rows)) {
...
function reboundCooldownRate(rows, targetDay, center, map = outcomeMapFromRows(rows)) {
...
function detectCrashCycle(rows) {
  const observedRows = rows.filter((row) => row.source !== 'forecast-path');
  let contiguousRows = [];
  for (const row of observedRows) {
    if (!contiguousRows.length
      || addDays(contiguousRows.at(-1).dateKey, 1) === row.dateKey) {
      contiguousRows.push(row);
    } else {
      contiguousRows = [row];
    }
  }
  const map = outcomeMapFromRows(contiguousRows);
  const clusterStarts = contiguousRows
    .filter((row) => !row.played && map.get(addDays(row.dateKey, -1)) !== false)
    .map((row) => row.dateKey)
    .slice(-5);
  if (clusterStarts.length < 3) {
    return { active: false, clusterStarts, reason: 'needs-three-cluster-starts' };
  }

  const intervals = clusterStarts.slice(1).map((dateKey, index) => (
    daysBetween(clusterStarts[index], dateKey)
  ));
  const periodEstimate = median(intervals);
  const period = Math.round(periodEstimate);
  if (period < 4 || period > 21 || Math.abs(periodEstimate - period) > 0.5) {
    return { active: false, clusterStarts, intervals, reason: 'period-out-of-range' };
  }
  const maximumDeviation = Math.max(...intervals.map((interval) => Math.abs(interval - period)));
  if (maximumDeviation > 1) {
    return { active: false, clusterStarts, intervals, period, reason: 'inconsistent-intervals' };
  }

  const consistency = 1 - maximumDeviation / period;
  const support = Math.min(1, intervals.length / 3);
  const confidence = Math.min(0.82, (0.58 + 0.18 * support) * consistency);
  const anchorDay = clusterStarts.at(-1);
  const firstAlignedDay = clusterStarts[0];
  const phaseStats = Array.from({ length: period }, () => ({ yes: 0, no: 0, total: 0 }));
  for (const row of contiguousRows) {
    if (row.dateKey < firstAlignedDay) continue;
    const phase = positiveModulo(daysBetween(anchorDay, row.dateKey), period);
    phaseStats[phase].total += 1;
    if (row.played) phaseStats[phase].yes += 1;
    else phaseStats[phase].no += 1;
  }
...
function crashCycleForecast(context, targetDay, center) {
  if (!context?.active) {
    return {
      active: false,
      confidence: 0,
      phase: null,
      probability: center,
      rawPhaseProbability: center
    };
  }
  const phase = positiveModulo(daysBetween(context.anchorDay, targetDay), context.period);
  const stats = context.phaseStats[phase] || { yes: 0, no: 0, total: 0 };
  const strength = 1.5;
  const rawPhaseProbability = (stats.yes + center * strength) / (stats.total + strength);
  return {
    active: true,
    anchorDay: context.anchorDay,
    clusterStarts: context.clusterStarts,
    confidence: context.confidence,
    intervals: context.intervals,
    period: context.period,
    phase,
    phaseObservations: stats.total,
    probability: clamp(
      (1 - context.confidence) * center + context.confidence * rawPhaseProbability
    ),
    rawPhaseProbability: clamp(rawPhaseProbability)
  };
}

function calculateCandidates(state, targetDay, { cycleContext = null } = {}) {
  const rows = answeredRowsBefore(state, targetDay);
  const map = outcomeMapFromRows(rows);
  const baseline = betaRate(rows);
  const cycle = crashCycleForecast(cycleContext || detectCrashCycle(rows), targetDay, baseline);
  ...
  const candidates = {
    ...
    transition: transitionRate(rows, targetDay, baseline, map),
    streak: streakRate(rows, targetDay, baseline, map),
    sinceLastYes: gapContextRate(rows, targetDay, baseline, 7, 7),
    weeklyCadence: weeklyCadenceRate(rows, targetDay, baseline, map),
    monthlyCadence: monthlyCadenceRate(rows, targetDay, baseline),
    crashCycle: cycle.rawPhaseProbability,
    recentLoad: recentLoadRate(rows, targetDay, baseline, map),
    reboundCooldown: reboundCooldownRate(rows, targetDay, baseline, map),
...
function computeForecast(state, targetDay, { cycleContext = null } = {}) {
  const outcomeRows = answeredRowsBefore(state, targetDay);
  const resolvedCycleContext = cycleContext || detectCrashCycle(outcomeRows);
  const candidates = calculateCandidates(state, targetDay, { cycleContext: resolvedCycleContext });
  const cycle = crashCycleForecast(resolvedCycleContext, targetDay, candidates.baseline);
  const rows = scoreRows(state, targetDay);
  let selection;
  let stage;

  if (rows.length < 14) {
    const weights = cycle.active
      ? { baseline: 1 - cycle.confidence, crashCycle: cycle.confidence }
      : { baseline: 1 };
    const probability = weightedProbability(candidates, weights);
    selection = {
      probability,
      weights,
      shadowProbability: probability,
      shadowWeights: weights,
      model: cycle.active ? 'cycle-aware-baseline' : 'baseline',
      diagnostics: null
    };
    stage = cycle.active ? 'cycle-aware-bayesian-baseline' : 'bayesian-baseline';
  } else if (rows.length < 28) {
    const cycleWeight = cycle.active ? Math.min(0.55, cycle.confidence * 0.75) : 0;
    const weights = {
      baseline: 0.7 * (1 - cycleWeight),
      calendar: 0.15 * (1 - cycleWeight),
      recency: 0.15 * (1 - cycleWeight),
      ...(cycleWeight ? { crashCycle: cycleWeight } : {})
    };
    const probability = weightedProbability(candidates, weights);
    selection = {
      probability,
      ...
      shadowProbability: probability,
      shadowWeights: weights,
      model: cycle.active ? 'cycle-aware-fixed-blend' : 'fixed-blend',
      diagnostics: null
    };
    stage = cycle.active ? 'cycle-aware-fixed-blend' : 'fixed-blend';
  } else {
...
const OUTLOOK_HORIZON = 30;
const OUTLOOK_PARTICLE_CAP = 32;
const OUTLOOK_METHOD = 'deterministic-systematic-particle-marginalization-v2-cycle-aware';

function computePatternOutlookForecast(state, targetDay, { cycleContext = null } = {}) {
  const outcomeRows = answeredRowsBefore(state, targetDay);
  const resolvedCycleContext = cycleContext || detectCrashCycle(outcomeRows);
  const candidates = calculateCandidates(state, targetDay, { cycleContext: resolvedCycleContext });
  const cycle = crashCycleForecast(resolvedCycleContext, targetDay, candidates.baseline);
  ...
  const contextualProbability = clamp(sigmoid(logit(shrunkPatternProbability) + contextShift));
  const cycleWeight = cycle.active ? Math.min(0.72, cycle.confidence) : 0;
  const probability = clamp(
    (1 - cycleWeight) * contextualProbability
    + cycleWeight * cycle.rawPhaseProbability
  );
  ...
  return {
    ...
    candidates,
    weights: {
      baseline: 0.70 * (1 - cycleWeight),
      calendar: 0.04 * (1 - cycleWeight),
      dayOfWeek: 0.04 * (1 - cycleWeight),
      recency: 0.04 * (1 - cycleWeight),
      weeklyCadence: 0.03 * (1 - cycleWeight),
      monthlyCadence: 0.03 * (1 - cycleWeight),
      transition: 0.03 * (1 - cycleWeight),
      streak: 0.03 * (1 - cycleWeight),
      recentLoad: 0.03 * (1 - cycleWeight),
      reboundCooldown: 0.03 * (1 - cycleWeight),
      ...(cycleWeight ? { crashCycle: cycleWeight } : {})
    },
    cycle,
    model: cycle.active ? 'cycle-aware-pattern-outlook' : 'strongly-shrunk-pattern-outlook'
  };
}
...
function normalizeBranches(branches) {
  ...
  if (!Number.isFinite(total) || total <= 0) {
    throw new Error('Outlook branches must have positive finite total weight.');
  }
  return branches.map((branch) => ({ ...branch, weight: branch.weight / total }));
}

function systematicResample(branches, particleCap) {
  const normalized = normalizeBranches(branches);
  if (normalized.length <= particleCap) {
    return { branches: normalized, resampled: false };
  }

  const ordered = [...normalized].sort((left, right) => left.path.localeCompare(right.path));
  const selected = [];
  let sourceIndex = 0;
  let cumulative = ordered[0].weight;
  for (let particleIndex = 0; particleIndex < particleCap; particleIndex += 1) {
    const quantile = (particleIndex + 0.5) / particleCap;
    while (sourceIndex < ordered.length - 1 && quantile > cumulative) {
      sourceIndex += 1;
      cumulative += ordered[sourceIndex].weight;
    }
    selected.push({
      ...ordered[sourceIndex],
      weight: 1 / particleCap
    });
  }
  return { branches: selected, resampled: true };
}

function buildRecursiveOutlook(
  state,
  baseDay,
  horizon = OUTLOOK_HORIZON,
  { particleCap = OUTLOOK_PARTICLE_CAP } = {}
) {
  if (!Number.isInteger(horizon) || horizon < 1) {
    throw new TypeError('Outlook horizon must be a positive integer.');
  }
  if (!Number.isInteger(particleCap) || particleCap < 2) {
    throw new TypeError('Outlook particle cap must be an integer of at least two.');
  }

  let branches = [{ state, weight: 1, path: '' }];
  const points = [];
  const cycleContext = detectCrashCycle(
    answeredRowsBefore(state, addDays(baseDay, 1))
  );
  ...
  if (typeof state.outcomes?.[baseDay]?.played !== 'boolean') {
    ...
    branches = [
      {
        state: cloneStateWithOutcome(state, baseDay, false),
        weight: 1 - currentProbability,
        path: '0'
      },
      {
        state: cloneStateWithOutcome(state, baseDay, true),
        weight: currentProbability,
        path: '1'
      }
    ];
  }
  ...
  for (let offset = 1; offset <= horizon; offset += 1) {
    ...
    const forecastByPath = new Map();
    const branchForecasts = branches.map((branch) => {
      let forecast = forecastByPath.get(branch.path);
      if (!forecast) {
        forecast = offset === 1
          ? computeForecast(branch.state, targetDay, { cycleContext })
          : computePatternOutlookForecast(branch.state, targetDay, { cycleContext });
        forecastByPath.set(branch.path, forecast);
      }
    ...
    const next = systematicResample(expandedBranches, particleCap);
    points.push({
      targetDay,
      probability: rounded.probability,
      percent: rounded.percent,
      branchCount: branchForecasts.length,
      uniqueBranchCount: forecastByPath.size,
      pathWeightTotal,
      expandedBranchCount: expandedBranches.length,
      nextBranchCount: next.branches.length,
